...
const char* iftttURL = "http://maker.ifttt.com/trigger/ESP_MOTION/json/with/key/dC9562ep-YFMBYK7Ybbt-Z";
const char* ssid = "virus394;          // Your WiFi Name.
const char* password = "aaaaaaaaa";  // Your WiFi Password.
char auth[] = "UCDZA-WRQc3SNlBJD6n7iLWHI3oBSsd6";      // Your Blynk Authentication token.
...
WidgetLED led1(V1);         // Virtual LED on Blynk App (Configure with V0)

BLYNK_CONNECTED() {         // Syncing Hardware with Blynk Server when booting up.
  Blynk.syncAll();  
}

BLYNK_WRITE(V2) {           // Retrieve Data from Button to the Hardware Via Virtual pin 1.  
  state = param.asInt();
}
